...
class MovieIndex extends Component
{
    ...
    public function generateMovie()
    {
        ...
        if ($apiMovie->successful()) {
            ...
            /* 
                Take the array of genre from the movie api 
            */
            $tmdb_genres = $newMovie['genres'];
                /*  Collect the ids of the array of genre  */
            $tmdb_genres_ids = collect($tmdb_genres)->pluck('id');

            /* Search for genre from the database by genre id */

            $genres = Genre::whereIn('tmdb_id', $tmdb_genres_ids)->get();

            

            /* Create genre for each created movie */
      ...
      public function render()
    {
        return view('livewire.movie-index', [
            'movies' => Movie::query()->where('title', 'LIKE', "%{$this->search}%")
