...
class GenreIndex extends Component
{
    ...
    {
        $newGenre = Http::get("https://api.themoviedb.org/3/genre/movie/list?api_key=8a11aac3fb4ef5f1f9607ee7e0329793&language=en-US")->json();
    
        foreach($newGenre['genres'] as $genre){
                Genre::create([
                'tmdb_id' => $genre['id'],
                'title'    => $genre['name'],
    ...
    public function render()
    {
        return view('livewire.genre-index', [
            'genres' => Genre::query()->where('title', 'LIKE', "%{$this->search}%")
